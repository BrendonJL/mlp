...
- Fix: Changed `src/training/train` → `src.training.train` (dots not slashes!)
...
**3. Missing stable-baselines3** ✅
- Error: `ModuleNotFoundError: No module named 'stable_baselines3'`
- Fix: `poetry add stable-baselines3`
- Bonus error: Typo `stable-baseline3` (singular) → `stable-baselines3` (plural!)

**4. Config Key Mismatches (4 separate issues!)** ✅
- `config["environment"]["name"]` → `config["environment"]["game"]`
- `config["hyperparameters"]` → `config["dqn_hyperparameters"]`
- `config["experiment"]["policy"]` → `config["dqn_hyperparameters"]["policy"]`
- `config["paths"]["models"]` → `config["paths"]["model_dir"]`
...
**9. Image Format Incompatibility** ✅
...
**11. Gym API Compatibility - Parameters** ✅
...
**12. Gym API Compatibility - Return Values** ✅
...
env = JoypadSpace(env, SIMPLE_MOVEMENT)      # 256 → 7 actions
env = CompatibilityWrapper(env)              # Old/new Gym API bridge
env = GrayscaleWrapper(env)                  # RGB → Grayscale
env = ResizeWrapper(env)                     # 240×256 → 84×84
# NormalizeWrapper REMOVED - SB3 handles it
env = FrameStackWrapper(env, num_stack=4)    # Stack 4 frames
env = TransposeWrapper(env)                  # (H,W,C) → (C,H,W)
# Final: (4, 84, 84) uint8 [0, 255] ✅
```
...
✅ PostgreSQL logging working (experiment_id: 9)
✅ Model saved: `models/dqn_baseline_world1-1_final`
✅ Git metadata tracked: 4ce6ec4a, Python 3.11.14, PyTorch 2.9.1+cu128
...
2. **GrayscaleWrapper** - RGB → grayscale
...
**Root Cause**: Library ecosystem transition (Gym → Gymnasium) incomplete
...
- ✅ End-to-end test run (1000 timesteps)
- ⏳ Full training (2M timesteps) - starting now!
- ⏳ Evaluation script
- ⏳ Analysis notebook

**Connection to cybersecurity goals**:
- Dependency management → security tool deployment
- API compatibility → integrating security products
- Metadata tracking → incident investigation
- Defensive programming → vulnerability prevention
- Systematic debugging → incident response
...
## 🚀 Afternoon Session: Full Training Run & 4 More Bugs!

**Timeline**: 9:58 AM - 12:00 PM (Full pipeline debugging)

### The 4 Additional Bugs Fixed

**Bug #13: Missing tqdm/rich Dependencies** ✅
...
**Bug #14: NumPy Type Incompatibility with PostgreSQL** ✅
...
- Learning: Type conversions needed at system boundaries (ML → Database)
- **THIS is why the 1000-step test didn't catch it!** No episodes completed, so database logging never ran!

**Bug #15: Database Schema Column Name Mismatch** ✅
...
**Bug #16: Missing timestamp Column** ✅
...
- 1000-step test: ✅ Found 12 bugs (imports, config, environment, wrappers)
- 1000-step test: ❌ Missed 4 bugs (database logging, type conversion, schema mismatch)
...
- Episode 5: ✅ **PIPELINE FULLY VALIDATED!**
...
1. ✅ Training orchestrator (`train.py`)
2. ✅ DQN agent with CnnPolicy (Stable-Baselines3)
3. ✅ Mario environment with 5 wrappers (Compatibility, Grayscale, Resize, FrameStack, Transpose)
4. ✅ Custom callbacks (WandbCallback, DatabaseCallback)
5. ✅ PostgreSQL episode logging (13 metrics per episode)
6. ✅ Weights & Biases cloud tracking (real-time graphs)
7. ✅ Progress bar with tqdm + rich (ETA, speed, completion %)
8. ✅ Model checkpointing every 100k steps
9. ✅ Console stats every 4 episodes
...
## 🧠 What I Learned (Afternoon Session)

### Integration Testing Principles

**Code Path Coverage**: Short tests can miss bugs that only trigger under realistic workloads
- Test ran 1000 steps: ✅ Environment setup, ❌ Episode completion
- Full run hit first episode at 28k: ✅ Found database bugs immediately
...
- ✅ Questioned test coverage ("shouldn't 1000 steps have caught this?")
- ✅ Understood type system boundaries (NumPy vs Python vs PostgreSQL)
- ✅ Recognized integration patterns (callbacks, wrappers, adapters)
- ✅ Made pragmatic decisions (kill hypridle vs complicated alternatives)
- ✅ Systematic debugging (no random guessing, methodical fixes)
...
## 💻 Code Modified (Afternoon Session)

**Files Modified:**
- `src/training/callbacks.py` - Added NumPy → Python type conversions for all 13 metrics
...
**Celebration Moment**: When episode 5 logged successfully at 12:00 PM! 🎉

**Connection to Cybersecurity Goals**:
- Integration testing → penetration testing methodology
- Type system boundaries → input validation & sanitization
- Schema evolution → database security & migration safety
- Process isolation → container security & privilege separation
- Systematic debugging → incident response & root cause analysis
...
**🎮 Mario is learning! The pipeline works! Phase 3 almost complete! 🚀**
